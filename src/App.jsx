...
// --- names (with your 4 custom names first) ---
const NAMES = [
  "Tony L",            // custom
  "Roberto",           // custom
  "Alberto",           // custom
  "Blanca",            // custom
  "Jordan Brown","Casey Miller","Drew Davis","Morgan Wilson","Jamie Moore",
  "Cameron Taylor","Reese Anderson","Quinn Thomas","Avery Jackson","Riley White",
  "Skyler Harris","Peyton Martin","Harper Thompson","Logan Garcia","Charlie Martinez",
  "Emerson Robinson","Finley Clark","Hayden Rodriguez","Rowan Lewis","Sawyer Lee",
  "Parker Walker","Dakota Hall","Jesse Allen","Kendall Young","Blake King","Micah Wright","River Scott"
];
...
export default function App() {
  ...
  // --- timer (RAF version) ---
  const [timer, setTimer] = useState(0);           // seconds, one-decimal display
  ...
  const startTimer = () => {
    ...
    const loop = () => {
      ...
      const elapsed = (now - startedAt.current) / 1000; // seconds
      setTimer(Number(elapsed.toFixed(1)));
  ...
  // --- ORIGINAL difficulty behavior (unchanged) ---
  const generateDeposit = (difficulty) => {
    ...
    return parseFloat((Math.random() * 290 + 10).toFixed(2)); // hard
  };
  ...
  const generateScenario = (diff = difficulty) => {
    // pick casino and bonus type (unchanged logic)
    const casino = Math.random() < 0.75 ? "RS" : (Math.random() < 0.5 ? "Golden" : "Golden GV");
    const bonusType =
      (casino === "RS" && (diff === "easy" ? Math.random() < 0.7 : Math.random() < 0.5))
        ? "Welcome"
        : (Math.random() < 0.5 ? "Regular" : "VIP");
    ...
    const fsWinnings =
      diff === "easy" ? 0
      : diff === "medium" ? pick([0, 5, 10, 15, 20, 25, 50, 75, 100, 150, 199])
      : parseFloat((Math.random() * 198.99 + 1).toFixed(2));

    const { bonus, total } = computeTotal({ casino, isWelcome, deposit, bonusPct, fsWinnings });

    const fsText =
      fsWinnings > 0
        ? ` I also won ${formatMoney(fsWinnings)} from free spins.`
        : "";

    const introText = isWelcome
      ? `for my ${bonusPct === 275 ? "first time depositing" : "welcome bonus"} (${bonusPct}%)`
      : `and claimed a ${bonusPct}% ${bonusType.toLowerCase()} bonus`;

    const customerName = pick(NAMES);

    setPrompt(
      `💬 ${customerName}: Hi, I just deposited ${formatMoney(deposit)} at ${casino} Casino ${introText}. Can you tell me my total wagers?${fsText}`
    );
    setAnswer("");
    setFeedback(null);
    ...
    setBreakdown(
      ...
      `✅ Total wagers: ${formatMoney(Number(total.toFixed(2)))}`
    );

    // restart RAF timer cleanly
    stopTimer();
    setTimer(0);
    startTimer();
  ...
  const submit = () => {
    ...
    // stop timer on submit (same behavior as your “perfect” version)
    stopTimer();

    // update best time ONLY if correct (your earlier requirement)
    if (ok && currentCasino) {
      setBestTimes((prev) => {
        const prior = prev?.[currentCasino];
        const updated = { ...prev, [currentCasino]: (prior == null || timer < prior) ? timer : prior };
  ...
  const handleDifficultyChange = (e) => {
    ...
    setDifficulty(next);
    generateScenario(next);
  };

  return (
    <div
      className="p-6 max-w-3xl mx-auto text-stone-100"
      style={{ background: "linear-gradient(135deg, #1f1f1f, #2b2b2b)", minHeight: "100vh", position: "relative" }}
    >
      ...
      <div className="text-sm mb-4 flex flex-wrap gap-4 items-center bg-gray-800 p-3 rounded-lg shadow">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <select
            ...
            title="Changing difficulty starts a new question"
          >
        ...
        <>
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              className="border p-2 flex-1 text-black rounded shadow"
              placeholder="Enter total wager requirement"
