...
const NAMES = [
  "Alex Johnson", "Tony L.", "Jordan Brown", "Casey Miller", "Drew Davis",
  "Morgan Wilson", "Jamie Moore", "Cameron Taylor", "Reese Anderson", "Quinn Thomas",
  "Avery Jackson", "Riley White", "Skyler Harris", "Blanca", "Harper Thompson",
  "Brian", "Charlie Martinez", "Roberto", "Finley Clark", "Hayden Rodriguez",
...
export default function WagerTrainer() {
  ...
  useEffect(() => {
    if (running) {
      ticker.current = setInterval(
        () => setTimer((t) => Number((t + 0.1).toFixed(1))),
        100
      );
    }
    return () => {
  ...
  const generateDeposit = (difficulty) => {
    ...
    if (difficulty === "easy")
      return pick([10, 50, 75, 100, 150, 200, 250, 300]);
    if (difficulty === "medium")
      return pick(Array.from({ length: 59 }, (_, i) => (i + 2) * 5));
  ...
  const generateBonusPercent = (casino, isWelcome, bonusType) => {
    ...
    if (bonusType === "Regular")
      return 10 + Math.floor(Math.random() * ((170 - 10) / 5 + 1)) * 5;
    if (bonusType === "VIP")
      return 150 + Math.floor(Math.random() * ((400 - 150) / 5 + 1)) * 5;
    return 0;
  };
  ...
  const generateScenario = (diff = difficulty) => {
    ...
    const bonusType =
      casino === "RS" &&
      (diff === "easy" ? Math.random() < 0.7 : Math.random() < 0.5)
        ? "Welcome"
        : Math.random() < 0.5
        ? "Regular"
        : "VIP";
    ...
    let fsWinnings =
      diff === "easy"
        ? 0
        : diff === "medium"
        ? pick([0, 5, 10, 15, 20, 25, 50, 75, 100, 150, 199])
        : parseFloat((Math.random() * 198.99 + 1).toFixed(2));
    const { bonus, total } = computeTotal({
      casino,
      isWelcome,
      deposit,
      bonusPct,
    ...
    const introText = isWelcome
      ? `for my ${
          bonusPct === 275 ? "first time depositing" : "welcome bonus"
        } (${bonusPct}%)`
      : `and claimed a ${bonusPct}% ${bonusType.toLowerCase()} bonus`;
    const customerName = pick(NAMES);
    setPrompt(
      `💬 ${customerName}: Hi, I just deposited ${formatMoney(
        deposit
      )} at ${casino} Casino ${introText}. Can you tell me my total wagers?${fsText}`
    );
    setAnswer("");
    setFeedback(null);
    ...
    setBreakdown(
      `🏢 Casino: ${casino}\n🎁 Bonus type: ${
        isWelcome ? "Welcome bonus" : `${bonusType} bonus`
      }\n💵 Deposit: ${formatMoney(deposit)}\n📈 Bonus %: ${bonusPct}%\n🎰 Free spin winnings: ${
        fsWinnings > 0 ? formatMoney(fsWinnings) : "none"
      }\n💲 Bonus amount: ${formatMoney(
        bonus
  ...
  return (
    ...
    >
      ...
      {started && (
        <>
          <p className="mb-4 text-lg bg-gray-900 p-4 rounded-lg shadow border border-gray-700">
            {prompt}
          </p>
          ...
          {feedback && (
            ...
            >
              {feedback === "correct"
                ? "✅ Correct!"
                : `❌ Incorrect. Correct answer was ${formatMoney(
                    Number(correctAnswer)
                  )}`}
            </p>
          )}
          {showBreakdown && (
            <pre className="mt-4 p-4 bg-gray-900 border border-gray-700 rounded-lg whitespace-pre-wrap shadow">
              {breakdown}
            </pre>
          )}
        </>
